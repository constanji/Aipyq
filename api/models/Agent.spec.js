...
describe('models/Agent', () => {
  ...
  describe('Agent CRUD Operations', () => {
    ...
    test('should handle ephemeral agent loading', async () => {
      ...
      const originalModule = jest.requireActual('aipyq-data-provider');
      ...
      jest.doMock('aipyq-data-provider', () => mockDataProvider);

      expect(agentId).toBeDefined();
      expect(endpoint).toBeDefined();

      jest.dontMock('aipyq-data-provider');
    });
  ...
  describe('Load Agent Functionality', () => {
    ...
    test('should test ephemeral agent loading logic', async () => {
      const { EPHEMERAL_AGENT_ID } = require('aipyq-data-provider').Constants;
