...
const fetchOpenAIModels = async (opts, _models = []) => {
  ...
  if (cachedModels) {
    // Use cached models but still merge with default models
    fetchedModels = cachedModels;
  } else if (baseURL || opts.azure) {
    ...
    if (fetchedModels.length > 0) {
      await modelsCache.set(baseURL, fetchedModels);
    }
  }
  
  // Always merge default models with fetched/cached models to ensure all models are available
  if (fetchedModels.length > 0) {
    // Combine default models and fetched models, removing duplicates
    const combinedModels = [...new Set([..._models, ...fetchedModels])];
  ...
  // Don't cache the merged models, only cache the fetched models from API
  // This ensures default models are always included even when cache exists
  return models;
};
